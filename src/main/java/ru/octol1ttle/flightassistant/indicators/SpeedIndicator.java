...
public class SpeedIndicator extends HudComponent {
    ...
    public void render(DrawContext context, TextRenderer textRenderer) {
        ...
        if (CONFIG.speed_showReadout) {
            ...
            drawFont(textRenderer, context, Text.translatable("flightassistant.ground_speed_short", String.format("%.2f", computer.velocityPerSecond.horizontalLength())), dim.lFrame + frameWidth * 0.25f, dim.hScreen * 0.8f, CONFIG.color);
            drawFont(textRenderer, context, Text.translatable("flightassistant.vertical_speed_short", String.format("%.2f", computer.velocityPerSecond.y)), dim.lFrame + frameWidth * 0.75f - 7, dim.hScreen * 0.8f, computer.velocityPerSecond.y <= -10.0f ? CONFIG.alertColor : CONFIG.color);
        }
