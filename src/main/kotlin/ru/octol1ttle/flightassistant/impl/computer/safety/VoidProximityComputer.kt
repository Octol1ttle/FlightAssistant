...
class VoidProximityComputer(computers: ComputerBus) : Computer(computers) {
    ...
    override fun <Response> handleQuery(query: ComputerQuery<Response>) {
        if (query is ThrustComputer.TargetThrustQuery && status <= Status.APPROACHING_DAMAGE_ALTITUDE) {
        ...
        if (query is PitchComputer.TargetPitchQuery && status <= Status.APPROACHING_DAMAGE_ALTITUDE) {
            query.respond(ControlInput(
                90.0f, Component.translatable("mode.flightassistant.vertical.void_escape"), ControlInput.Priority.HIGH,
                status = ControlInput.Status.fromBooleans(
                    status == Status.REACHED_DAMAGE_ALTITUDE && computers.thrust.current == 1.0f && !computers.thrust.noThrustSource,
